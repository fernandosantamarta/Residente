...
import { MEETING_TYPES, MEETING_STATUS_LABELS, VOTE_STATUS_LABELS } from '../../lib/voice'
    ...
    <div className="voice-wrap">
      ...
      {upcoming.length > 0 && (
        <section className="voice-section">
          ...
          {upcoming.map(m => (
            <ResidentMeetingRow key={m.id} meeting={m} onClick={() => setSelectedId(m.id)} />
          ))}
        </section>
      )}
...
function ResidentMeetingRow({ meeting: m, onClick }) {
  ...
  return (
    <button className={`voice-res-row${isPast ? ' past' : ''}`} onClick={onClick}>
