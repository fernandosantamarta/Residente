          ...
          {[...payable, ...review].map(v => {
            ...
            return (
              <div key={v.id} className="pay-fine-row">
                {/* Uniform row: title + amount, then a meta line (2 lines → equal
                    height for every fine); the action (buttons or badge) centered right. */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                    {underReviewRow ? (
                      <span style={{ fontSize: 11.5, fontWeight: 700, color: '#B54708', background: 'rgba(181,71,8,0.10)', border: '1px solid rgba(181,71,8,0.25)', padding: '5px 12px', borderRadius: 999, whiteSpace: 'nowrap' }}>{t('pay.fineUnderReview')}</span>
                    ) : (
                      <>
                        <button
                          type="button"
