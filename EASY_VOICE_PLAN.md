...
> **Build status — 2026-05-24.** Milestone 1 is shipped on branch
> `andres/easy-voice-nextjs` (7 commits ahead of `main`, pushed, not yet
> merged). All 13 Milestone 1 items are checked off (see §7). The
> Supabase backend (SQL, edge functions, secrets, DB webhook) is live;
> see `NEXT_SESSION.md` for the live-state log. Two pilot blockers
> remain before merge: replace placeholder `CONSENT_DISCLOSURES` copy
> in `lib/voice.ts` (lawyer review), and run an end-to-end smoke test
> through the real `/admin/voice/roster` → invite → onboard → vote →
> notice path. Resend-domain verification is cosmetic, not blocking.
...
All thirteen items below landed across Phases 1–4 on `andres/easy-voice-nextjs`. See §9 for the per-phase build log.

- [x] Multi-tenant association setup + configuration (Phase 1)
- [x] Owner roster CSV import (Phase 4 / Commit 1)
- [x] Owner onboarding + electronic voting consent collection (Phase 4 / Commit 3)
- [x] Meeting creation (all types) with FL notice deadline warnings (Phase 1)
- [x] Notice sending via email with delivery tracking (Phase 4 / Commit 4)
- [x] Document upload, display, and status management (draft/approved minutes) (Phase 1)
- [x] In-meeting open ballot voting (yes/no resolutions) (Phase 1)
- [x] In-meeting secret ballot voting (elections) — encrypted end-to-end in Phase 4 / Commit 5
- [x] Manual attendance tracking + quorum confirmation (Phase 1)
- [x] Multi-association login (one account, workspace switcher) (Phase 4 / Commit 6)
- [x] Florida compliance hard blocks and soft warnings (Phases 1 + 4 / Commit 3 — consent guard at DB level)
- [x] In-app + email notifications for all events (Phases 2 + 3 + 4 / Commit 4)
- [x] Owner-facing meeting archive (Phase 1)

### Milestone 2 — Full Feature Parity
*Goal: Complete election workflow, both proxy types, written ballots, SMS, QR check-in.*

- [ ] Full board election workflow (nominations → ballots → tiebreaker → results) — schema exists (`ev_candidates`); no workflow UI yet
- [ ] Uncontested election detection
- [ ] Limited proxy: submission, holder experience, auto-cast for fixed instructions — schema exists (`ev_proxies` + general-proxy-on-elections hard block trigger); no resident UI yet
- [ ] General proxy: submission with election hard block — DB hard block lives; submission UI pending
- [ ] Written ballot (asynchronous) voting — schema supports it (`ev_votes.mode = 'written_ballot'`); admin UI defaults to `in_meeting`
- [ ] SMS notifications (Twilio) — `NoticeChannel` type includes `sms`; no Twilio integration
- [ ] QR code check-in for in-person meetings — `ev_attendance.method` supports `self_checkin_qr`; no QR generator
- [x] **Admin delivery report (per-owner notice status)** — landed early as part of Phase 4 / Commit 4 (`ev_notices.delivery_report` jsonb + `ev_notice_recipients.email_status`)
- [ ] Management company cross-association dashboard — `ev_membership` table (Phase 4 / Commit 6) gives the data model; no UI yet

### Milestone 3 — Integration & Expansion
*Goal: Platform integrations, national expansion infrastructure, Easy Pay handshake.*

- [ ] Easy Pay delinquency → voting eligibility integration (schema in place: `residents.voting_eligible`)
...
- [ ] Encrypted ballot export/audit report (decryption + tally already runs client-side per Phase 4 / Commit 5; export tooling pending)
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 1 | **Co-owner primary voter** | Resolved (first-cast wins) | Enforced by the unique constraint `unique (vote_id, unit_number)` on `ev_ballots`. Second co-owner attempting to cast hits a `23505` and gets the message "Your unit has already cast a ballot for this vote." Revisit if any pilot HOA's bylaws require a pre-designated primary. |
| 2 | **Renter accounts** | Deferred | Still no renter accounts. Revisit if any target associations have bylaws granting renters participation rights. |
| 3 | **Asynchronous vote quorum** | Open | Written-ballot voting is not yet built (Milestone 2). Confirm the "X% of eligible owners return ballots by deadline" interpretation under FL law before building it. |
| 4 | **Electronic voting resolution workflow** | Open | `communities.electronic_voting_resolution_adopted` is a self-attested boolean today. No upload-the-resolution-as-proof UI. Decide before pilot whether self-attestation is acceptable or we require the PDF. |
| 5 | **Proxy holder identity verification** | Open (blocks proxy build) | Proxy submission UI isn't built (Milestone 2). When it is, decide: is the emailed proxy-check-in link sufficient, or do we need a PIN sent out-of-band? |
| 6 | **Easy Voice ↔ Easy Pay overlap** | Schema ready, integration pending | `residents.voting_eligible` exists. Easy Pay (a future module) will need to write it on delinquency. No retrofit needed. |
| 7 | **Consent disclosure copy** | **Pilot blocker** | `lib/voice.ts` → `CONSENT_DISCLOSURES` ships placeholder copy. Must be replaced with lawyer-authored FL-required disclosures before any real owner sees `/onboard`. `ev_consents` rows are immutable, so post-merge fixes mean re-onboarding. |
| 8 | **Resend domain verification** | Cosmetic, not blocking | Currently `NOTIFY_FROM_VOICE = "Residente <onboarding@resend.dev>"`. Verify `residente.io` in Resend (SPF + DKIM TXT records in Namecheap) and swap the secret to `notices@residente.io` for production deliverability. |
| 9 | **Tally-key custody** | Single-admin for Phase 4 | The admin who creates a secret vote holds the only password + key card. If they lose both, the ballots are unrecoverable — which is the legal intent of a secret ballot. A future enhancement could add Shamir 2-of-3 split between board officers. |

---

## 9. Build Log

Per-phase delivery history on `andres/easy-voice-nextjs`. The branch is 7 commits ahead of `main` as of 2026-05-24; not yet merged.

### Phase 1 — Scaffold (commit `491116c`, ported to Next.js in `a3e69f6`)
- `ev_meetings`, `ev_meeting_docs`, `ev_votes`, `ev_ballots` tables + RLS
- Admin meeting CRUD, open + secret ballot voting (plaintext)
- Manual attendance + quorum confirmation
- Resident-facing meeting list + detail at `/app/voice` + `/app/voice/[id]`
- FL notice-period soft warnings; elections-must-be-secret hard block
- Document upload to Supabase Storage with auth-gated reads

### Phase 2 — In-app notifications (commit `9d59633`)
- `ev_notices` + `ev_notice_recipients` tables with fan-out trigger
- Bell badge in left rail with realtime subscription + 60s poll fallback
- Bell panel dropdown (10 most recent), mark-as-read, click-to-navigate

### Phase 3 — Notifications inbox + audit log (commits `1cdc7c0`, `af29479`, `91ce49c`)
- `ev_audit_log` (append-only) + `lib/audit.ts` writer
- Auto-notice triggers for `vote_opened` and `vote_results`
- Full-page paged inbox at `/app/notifications` with kind filter, mark-all-read
- Security-definer fix on the read-count trigger

### Phase 4 — Pilot launch readiness (commits `9b2aa69` through `6d646e3` + `59b0818`)
*Closes all remaining Milestone 1 items. Driver: first Florida HOA pilot.*

| Commit | Hash | Summary |
|---|---|---|
| 1 | `9b2aa69` | Owner roster CSV import at `/admin/voice/roster` (preview table + per-row error badges + idempotent upsert) |
| 2 | `ce23ff2` | `voice-invite-owner` edge function + Invite/Re-invite buttons on roster |
| 3 | `a7e898c` | Public `/onboard` route (password → TOS → distinct consent screen) + `ev_ballot_consent_guard` DB hard block + `/api/ip` route |
| 4 | `627e54d` | `notice-email-fanout` edge function + extended `ev_notice_fanout()` to materialise email recipients + two-checkbox channel UI |
| 5 | `94a6928` | `lib/ballotCrypto.ts` (tweetnacl) + admin VoteForm secret-key flow (tally password + downloaded key card) + admin VoteRow close-and-decrypt flow + resident `cast()` secret-path |
| 6 | `6d646e3` | `ev_membership` join table + backfill + upsert trigger + `<CommunitySwitcher/>` component in both layouts |
| — | `59b0818` | Inline `corsHeaders` in `voice-invite-owner` so the function deploys cleanly from the Supabase dashboard editor (not just from the CLI) |

**Supabase backend live as of 2026-05-24:** All Phase 4 SQL run in production. Both edge functions deployed (`voice-invite-owner` with JWT verify on, `notice-email-fanout` with JWT verify off). Four secrets set (`RESEND_API_KEY`, `APP_URL`, `NOTIFY_FROM_VOICE`, `NOTICE_WEBHOOK_SECRET`). DB webhook `ev-notices-email-fanout` wired with `X-Webhook-Secret` header.

**Untested:** End-to-end smoke (CSV → invite → onboard → consent → vote → notice). The build passed at every commit but no human has run the full loop in production.

**Pre-merge checklist** (in §8 as items #7 and #8):
1. (Required) Replace `CONSENT_DISCLOSURES` placeholder copy in `lib/voice.ts` with lawyer-authored FL disclosures
2. (Strongly recommended) Run the end-to-end smoke test through the real UI with a 1-row roster targeting the operator's own email
3. (Optional, post-merge OK) Verify `residente.io` in Resend and swap `NOTIFY_FROM_VOICE` to `notices@residente.io`

### What's next after merge
Tackle Milestone 2 in priority order based on pilot feedback. The two highest-value items are full board elections and proxy submission — both have schema support already (`ev_candidates`, `ev_proxies`), so the work is mostly UI + workflow rather than data model.
